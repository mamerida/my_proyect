...
describe(" return moves to message" , ()=>{
    ...
    test("with row 0 column 0 and move up", ()=>{
        //[to_row,to_col]
        expect(returnCoordinates({moves:{up:"up"}},0,0)).toStrictEqual([1,0]);
    })

    test("with row 0 column 0 and move upJump", ()=>{
        //[to_row,to_col]
        expect(returnCoordinates({moves:{upJump:"upJump"}},0,0)).toStrictEqual([2,0]);
    })

    test("with row 4 column 4 and move upLeft", ()=>{
        //[to_row,to_col]
        expect(returnCoordinates({moves:{upLeft:"upLeft"}},4,4)).toStrictEqual([5,5]);
    })

    test("with row 4 column 4 and move upRigth", ()=>{
        //[to_row,to_col]
        expect(returnCoordinates({moves:{upRigth:"upRigth"}},4,4)).toStrictEqual([5,3]);
    })
